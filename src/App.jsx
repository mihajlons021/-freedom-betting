  ...
  return (
    <div style={{ padding: "16px" }}>
      ...
      <button onClick={handleSubmit} disabled={loading || !desc.trim()} style={{
        ...
        {done ? "✅  Bet Saved to Firebase!" : loading ? "Saving..." : `Lock ${amount.toLocaleString()} $FREE · Predict ${side}`}
      </button>
    ...
    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: "12px", overflow: "hidden" }}>
      ...
      <div style={{ padding: "14px" }}>
        ...
        {bet.status === "open" && onAccept && (
          <button onClick={() => onAccept(bet)} style={{ width: "100%", padding: "10px", background: `linear-gradient(135deg,${T.green},${T.greenDim})`, border: "none", borderRadius: "8px", color: "#0a1a0a", fontSize: "0.8rem", fontWeight: 800, cursor: "pointer" }}>
            Accept Bet →
          </button>
        )}
        {bet.status === "active" && onClaim && (
          <button onClick={() => onClaim(bet)} style={{ width: "100%", padding: "10px", background: `linear-gradient(135deg,${T.purple},${T.purpleDim})`, border: "none", borderRadius: "8px", color: "white", fontSize: "0.8rem", fontWeight: 800, cursor: "pointer" }}>
            🏆 Claim Victory
          </button>
        )}
                      ...
                      <div style={{ display: "flex", gap: "8px", marginBottom: "14px" }}>
                        {["YES","NO"].map(s => (
                          <button key={s} onClick={() => setClaimSide(s)} style={{
                            ...
                            borderRadius: "8px", color: claimSide === s ? (s === "YES" ? T.green : T.red) : T.muted,
...
function Leaderboard({ bets }) {
  ...
  const data = Object.values(stats).sort((a,b) =>
    tab === "streak" ? b.wins - a.wins : b.tokens - a.tokens
  ).slice(0, 5).map((e,i) => ({ ...e, rank: i+1 }));
            ...
            {podiumOrder.map((e,i) => {
              ...
              return (
                <div key={e.rank} style={{ flex:1, textAlign:"center" }}>
                  ...
                  <div style={{ height:`${podiumHeights[i]}px`, background:`linear-gradient(180deg,${podiumColors[i]}22,${podiumColors[i]}0a)`, border:`1px solid ${podiumColors[i]}44`, borderRadius:"8px 8px 0 0", display:"flex", alignItems:"center", justifyContent:"center", flexDirection:"column", gap:"2px" }}>
...
export default function App() {
  ...
  const username = "Player_" + Math.random().toString(36).slice(2,6).toUpperCase();
